...
class AutoShootNode(Node):
    def __init__(self):
        ...
        self.x_pid = PID.PID(P=0.15, I=0.00, D=0.0007)
        self.y_pid = PID.PID(P=0.15, I=0.00, D=0.0007)
    ...
    def process_image(self):
        ...
        # ===== 新增：在画面上实时显示坐标数值 =====
        # 参数含义：画面, 字符串, 坐标, 字体, 字号, 颜色, 粗细
        cv2.putText(frame, f"X: {self.CenterX}  Y: {self.CenterY}", 
                    (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
        
        # 如果当前没有找到球 (CenterX 为 -1)，显示一个警告
        if self.CenterX < 0:
            cv2.putText(frame, "LOST BALL", (10, 60), 
