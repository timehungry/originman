...
class Board:
    ...
    def __init__(self, device="/dev/ttyS1", baudrate=1000000, timeout=5):
        ...
        self.asr_queue = queue.Queue(maxsize=1)
        ...
        self.parsers = {
            ...
            PacketFunction.PACKET_FUNC_ASR: self.packet_report_asr,
        }

        threading.Thread(target=self.recv_task, daemon=True).start()
        time.sleep(0.1)

    def packet_report_asr(self, data):
        try:
            self.asr_queue.put_nowait(data)
        except queue.Full:
            pass
    ...
    def get_asr_data(self):
        if self.enable_recv:
            try:
                data = self.asr_queue.get(block=False)
                return data
            except queue.Empty:
                return None
        else:
    ...
    def set_oled_text(self, line, text):
        data = [line, len(text)] # 子命令为 0x01 设置 SSID, 第二个字节是字符串长度，该长度包含'\0'字符串结束符(sub-command is 0x01 set SSID, the second byte is the string length, the length involves '\0' end string)
